...
This channel updates only the core. The Plugin updates panel in Version management lists installed plugins from their verified current registry. It pins the displayed version and source, leaves disabled plugins untouched, and checks runtime registration after installation. Refresh distinguishes a loaded version from a pending reload. Any requested restart waits for an explicit user action. Plugin installation shares the core operation lock so it cannot overlap core replacement or a restart. New plugin installation and source registration remain separate configuration tasks. Official app self-update and bundled-core replacement are blocked while the reviewed channel is selected because they may remove compatibility patches. macOS signature and Gatekeeper validation remain unchanged. Automatic reviewed app replacement still requires a trusted signed app distribution, or the patches to be included upstream. Selecting **Use official channel** explicitly removes this guard.
